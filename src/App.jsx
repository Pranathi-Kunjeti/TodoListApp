      ...
      <div className="mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[80vh] md:w-[50%]">
        ...
        <div className="addTodo my-5 flex flex-col gap-4">
          ...
          <div className="flex">
            ...
            <button onClick={handleAdd} disabled={todo.length<=0} className="bg-violet-800 mx-2 rounded-full hover:bg-voilet-950 disabled:bg-violet-500 p-4 py-2 text-sm font-bold text-white">Save</button>
          </div>
        </div>
            ...
            {todos.map(item=>{
              return(showFinished||!item.isCompleted)&&<div key={item.id} className={"todo flex my-3 justify-between"}>
                ...
                <div className="buttons flex h-full">
                  ...
                  <button onClick={(e)=>handleDelete(e,item.id)} className="bg-violet-800 hover:bg-violet-950 p-2 py-1 text-sm font-bold text-white rounded-md mx-1"><AiFillDelete />
                  </button>
              </div>
              </div>
            })}
